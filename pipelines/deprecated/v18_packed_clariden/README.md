...
> (`../../v18/packed/`), not Clariden; this port was written and sized but never adopted. Paths
> below still name its old location `pipelines/v18/packed_clariden/`. Kept for historical
> reference only -- do not use for new runs.

# Packed whole-node v18 grid postprocessing for Clariden (CSCS) — esub-free

Simpler alternative to `../v18_packed_clariden_esub/`. Same idea (pack ~36 cosmologies onto each
...
srun -A a0158 -p normal -N1 --ntasks=1 --time=00:30:00 --environment=tensorflow bash -c '
  ...
  python -u /users/athomsen/dlss/repos/multiprobe-simulation-forward-model/pipelines/v18/packed_clariden/run_cosmo.py \
...
- Per-index logs land in `$LOG_DIR/index<idx>.log`; the node-level srun log is
  `$LOG_DIR/tfr_grid_v18_<jobid>_<element>.out`.
