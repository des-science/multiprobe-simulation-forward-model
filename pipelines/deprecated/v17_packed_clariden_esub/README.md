...
> (`../../v17/packed/`), not Clariden; this port was written and sized but never adopted. The
> esub-free sibling `../v17_packed_clariden/` superseded it even within the Clariden experiment.
> Paths below still name its old location `pipelines/v17/packed_clariden_esub/`. Kept for
> historical reference only -- do not use for new runs.

# Packed whole-node v17 grid postprocessing for Clariden (CSCS)

Clariden analogue of `pipelines/v17/packed/` (which was the Perlmutter regular-QOS fallback).
...
srun -A a0158 -p normal -N1 --ntasks=1 --time=00:30:00 --environment=tensorflow bash -c '
  ...
  esub /users/athomsen/dlss/repos/multiprobe-simulation-forward-model/msfm/apps/run_grid_postprocessing.py \
    ...
    --job_name=tfr_grid_v17 --log_dir=/iopsstor/scratch/cscs/athomsen/run_files/v17/esub_logs'
```
...
- **Bookkeeping**: finished indices are appended to `<log_dir>/tfr_grid_v17_done.dat`. To rerun
...
- **Merge completeness**: `merge()` batches cosmologies by file order — it requires the complete
  ...
  `wc -l tfr_grid_v17_done.dat` == 2500 before merging.
